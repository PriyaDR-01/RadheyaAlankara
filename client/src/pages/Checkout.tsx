...
export default function CheckoutPage() {
  ...
  const handleRazorpayPayment = async (orderData: CheckoutFormData) => {
    try {
      ...
      const { id: orderId, amount, currency, key: keyId } = await orderResponse.json();      const options = {
        key: keyId,
        amount: amount,
        currency: currency,
        name: 'Radheya Alankara',
        description: 'Jewelry Purchase',
        order_id: orderId,        handler: async (response: RazorpayResponse) => {
          await verifyPaymentAndCreateOrder(orderData, response);
        },
        prefill: {
        ...
        theme: {
          color: '#6B4C20', // Converting HSL to hex for Razorpay
        },
      };
  ...
  const verifyPaymentAndCreateOrder = async (formData: CheckoutFormData, paymentResponse: RazorpayResponse) => {
    try {
      const orderData = {
      ...
      const response = await apiRequest('POST', '/api/razorpay/verify', orderData);
